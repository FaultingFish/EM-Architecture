...
- `systemd/` - app services bound to localhost only.
- `caddy/Caddyfile` - local reverse proxy for `emfi.ics.red`.
...
Control owns dangerous hardware. Any public route that can reach Control must be protected by Cloudflare Access or equivalent authentication and authorization. Do not expose ports `8001`, `8002`, `8003`, or the local Caddy listener directly to the internet.
